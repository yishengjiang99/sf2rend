...
const {
	...
	VCF_MOD_PITCH,
	VCF_MOD_FC,
	volumecoarse, pitchbendcoarse, expressioncoarse, pancoarse} = midi_effects;
...
export function mk_vca_ctrl(ch, port) {
	...
	for (let i = VCA_ATTACK_TIME;i <= VCA_RELEASE_TIME;i++) {
		divbox.push(mkknob({title: inputnames.shift(), defval: defaults.shift(), "data-mcc": i, oninput: (e) => post_val(e.target.dataset['mcc'], e.target.value)}));
	}
	const g = mkdiv("fieldset", {style: knob_set_style}, divbox);
	return g;
}
export function mk_vcf_ctrl(ch, port) {
	const from = VCF_ATTACK_TIME, to = VCF_MOD_FC;
...
function mk_midi_cc_knob(divbox, titles, i, defaults, post_val) {
	divbox.push(mkknob({title: titles[i], defval: defaults[i], "data-midi-cc": i, oninput: (e) => post_val(e.target.dataset['midi-cc'], e.target.value)}));
}

export function mk_attenuate(ch, port) {
	const post_val = (cc_num, val) => port.postMessage(new Uint8Array([midi_ch_cmds.continuous_change, ch, cc_num, val]));
	return mkdiv("fieldset", {style: knob_set_style}, [
		mkdiv("legend", "modulate"),
		mkknob({
			title: "filterFc", min: 1000,
			id: "filterFC",
			max: 30000,
			step: 10,
			class: "input-knob",
