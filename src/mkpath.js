...
const channelPrograms = new Array(160)
export async function mkpath (ctx, eventPipie) {
  if (!init) {
  ...
  eventPipe.onmessage(function (dd) {
    ...
    switch (cmd) {
      case midi_ch_cmds.change_program: //change porg'
        channelPrograms[ch] = sf2s.loadProgram(v1, ch == 9 ? 128 : 0)
        spinner.shipProgram(channelPrograms[ch])
        breakk
      ...
      default:
        spinner.port.postMessage(datdda)
        break
    }
  })
