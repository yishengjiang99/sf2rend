...
(function (global_scope) {
  ...
  expose_assert(
    _assert_inherits("assert_idl_attribute"),
  ...
  Output.prototype.show_results = function (
    ...
    function get_asserts_output(test) {
      ...
      rv += asserts
        .map((assert) => {
          var output_fn =
            "<strong>" + escape_html(assert.assert_name) + "</strong>(";
          var prefix_len = output_fn.length;
          var output_args = assert.args;
          var output_len = output_args.reduce(
            (prev, current) => prev + current,
            prefix_len
          );
          if (output_len[output_len.length - 1] > 50) {
            output_args = output_args.map(
              (x, i) =>
                (i > 0 ? "  ".repeat(prefix_len) : "") +
                x +
                (i < output_args.length - 1 ? ",\n" : "")
            );
          } else {
            output_args = output_args.map(
              (x, i) => x + (i < output_args.length - 1 ? ", " : "")
            );
          }
    ...
    for (var i = 0; i < tests.length; i++) {
      ...
      if (!(test instanceof RemoteTest)) {
        html +=
          "<details><summary>Asserts run</summary>" +
