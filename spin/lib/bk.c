...
float cent2freq(short cent) { return powf(2, (float)cent / 1200.f) * 8.176; }

void default_inst() {}
typedef struct _v {
  pgen_t *global_pzone;
  int global_zone_gen_cnt;
  pgen_t *pzone;
  int pzone_gen_cnt;
...
void loopzone(phdr *phr, int midi, int velocity) {
  ...
  short pzone[60], izone[60];
  for (int j = phr->pbagNdx; j < (phr + 1)->pbagNdx; j++) {
    pbag *pb = pbags + j;
    int n_pg = pb[j + 1].pgen_id - pb->pgen_id;

    pgen_t *pg = &pgens[pb->pgen_id];
    pgen_t *lastpg = &pgens[pb->pgen_id + n_pg];
    printf("\n\t fist pg %d-%s", pg->genid, generator[pg->genid]);
    printf("\t last pg %d-%s", lastpg->genid, generator[lastpg->genid]);

    if (lastpg->genid != Instrument) {
      vzone->global_pzone = pg;
      vzone->global_zone_gen_cnt = n_pg;
      printf("\nglobal found");
      continue;
    }
    instID = lastpg->val.uAmount;
    inst *ihead = insts + instID;
    printf("\n\t--- Inst %s: %d----", ihead->name, instID);
    continue;
    ...
    for (int ibg = ibgId; ibg < lastibg; ibg++) {
      ibag *ibgg = ibags + ibg;
      pgen_t *lastig =
          ibg < nibags - 1 ? igens + (ibgg + 1)->igen_id : igens + nigens - 1;
