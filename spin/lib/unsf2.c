 ...
 * will be assumed. */
#if defined(UNSF_BUILD) && \
    defined(DLL_EXPORT) /* building library as a dll for windows */
#define UNSF_SYMBOL __declspec(dllexport)
#elif defined(UNSF_BUILD) || \
    defined(UNSF_STATIC) /* building or using static lib for windows */
#define UNSF_SYMBOL
#else /* using library dll for windows */
#define UNSF_SYMBOL __declspec(dllimport)
#endif
#elif defined(UNSF_BUILD)
#if defined(SYM_VISIBILITY) /* __GNUC__ >= 4, or older gcc with backported \
                               feature */
...
static char *unsf_strtok_r(char *s1, const char *s2, char **ptr) {
  const char *p = s2;
...
static const unsigned int freq_table[UNSF_RANGE] = {
    ...
    4186009, 4434922, 4698636, 4978032, 5274041,  5587652,  5919911,  6271927,
    6644875, 7040000, 7458620, 7902133,

    8372018, 8869844, 9397273, 9956063, 10548082, 11175303, 11839822, 12543854};

static const double bend_fine[256] = {1,
                                      1.0002256593050698,
                                      1.0004513695322617,
                                      1.0006771306930664,
                                      1.0009029427989777,
...
static const double bend_coarse[UNSF_RANGE] = {1,
                                               1.0594630943592953,
                                               1.122462048309373,
                                               1.189207115002721,
                                               1.2599210498948732,
...
static int get8(FILE *f) { return getc(f); }
...
static void print_sf_string(UnSF_Options *options, FILE *f, const char *title,
                            int opt_no_write, SampleBank *samplebank) {
  char buf[256];
  char ch;
  int i = 0;
...
static void record_velocity_range(UnSF_Options *options,
                                  SampleBank *sample_bank, int drum,
                                  int banknum, int program, int velmin,
                                  int velmax, int type) {
  int i, count;
  ...
  if (!vlist) {
    ...
    if (drum)
      sample_bank->drum_velocity[banknum][program] = vlist;
    else
      sample_bank->voice_velocity[banknum][program] = vlist;
    vlist->range_count = 0;
  }
  ...
  if (type == RIGHT_SAMPLE)
    vlist->right_patches[i]++;
  else if (type == LEFT_SAMPLE)
    vlist->left_patches[i]++;
  else if (type == MONO_SAMPLE)
  ...
  if (options->opt_veryverbose)
    printf("%s#%d velocity range %d-%d for %s chan %s has %d patches\n",
           (i == count) ? "new " : "", vlist->range_count, velmin, velmax, name,
           (type == LEFT_SAMPLE)    ? "left"
           : (type == RIGHT_SAMPLE) ? "right"
                                    : "mono",
           (type == RIGHT_SAMPLE)  ? vlist->right_patches[i]
           : (type == LEFT_SAMPLE) ? vlist->left_patches[i]
                                   : vlist->mono_patches[i]);
}

/* gets facts and names */
static int grab_soundfont_banks(UnSF_Options *options, int sf_num_presets,
                                sfPresetHeader *sf_presets,
                                sfPresetBag *sf_preset_indexes,
                                sfGenList *sf_preset_generators,
                                sfInst *sf_instruments,
                                sfInstBag *sf_instrument_indexes,
                                sfGenList *sf_instrument_generators,
                                sfSample *sf_samples, SampleBank *sample_bank) {
  sfPresetBag *pindex;
  sfGenList *pgen;
  sfInst *iheader;
  sfInstBag *iindex;
  ...
  for (pnum = 0; pnum < sf_num_presets; pnum++) {
    int global_preset_layer, global_preset_velmin, global_preset_velmax,
        preset_velmin, preset_velmax;
    int global_preset_keymin, global_preset_keymax, preset_keymin,
        preset_keymax;
    ...
    if (pnum < sf_num_presets - 1)
      pindex_count =
          sf_presets[pnum + 1].wPresetBagNdx - sf_presets[pnum].wPresetBagNdx;

    if (pindex_count < 1) continue;
    ...
    for (inum = 0; inum < pindex_count; inum++) {
      int global_instrument_layer, global_instrument_velmin,
          global_instrument_velmax, instrument_velmin, instrument_velmax;
      int global_instrument_keymin, global_instrument_keymax, instrument_keymin,
          instrument_keymax;
      ...
      if ((pgen_count > 0) &&
        ...
        for (lnum = 0; lnum < iindex_count; lnum++) {
          ...
          if (global_instrument_velmin >= 0)
            instrument_velmin = global_instrument_velmin;
          if (global_instrument_velmax >= 0)
            instrument_velmax = global_instrument_velmax;
          if (global_instrument_keymin >= 0)
            instrument_keymin = global_instrument_keymin;
          if (global_instrument_keymax >= 0)
            instrument_keymax = global_instrument_keymax;

          if ((igen_count > 0) &&
              (igen[igen_count - 1].sfGenOper != SFGEN_sampleID))
            global_instrument_layer = TRUE;
          else
            global_instrument_layer = FALSE;
          ...
          if (instrument_velmin >= 0)
            velmin = instrument_velmin;
          else
            velmin = 0;
          if (instrument_velmax >= 0)
            velmax = instrument_velmax;
          else
            velmax = 127;
          ...
          if (instrument_keymin >= 0)
            keymin = instrument_keymin;
          else
            keymin = 0;
          if (instrument_keymax >= 0)
            keymax = instrument_keymax;
          else
            keymax = 127;
          ...
          if ((igen_count > 0) &&
            ...
            if (sample->sfSampleType & 0x8000 && options->opt_verbose) {
              ...
              if (options->opt_veryverbose) printf("\n");
              continue;
            }
            ...
            if (drum) {
              ...
              for (pool_num = keymin; pool_num <= keymax; pool_num++) {
                ...
                if (!sample_bank->drum_name[options->opt_drum_bank][drumnum]) {
                  sample_bank->drum_name[options->opt_drum_bank][drumnum] =
                      strdup(s);
                  if (options->opt_verbose)
                    printf("drumset #%d drum #%d %s\n", options->opt_drum_bank,
                           drumnum, s);
                }
                if (sample->sfSampleType == LEFT_SAMPLE)
                  ...
                  sample_bank
                      ->drum_samples_mono[options->opt_drum_bank][drumnum]++;
                record_velocity_range(options, sample_bank, drum,
                                      options->opt_drum_bank, drumnum, velmin,
                                      velmax, sample->sfSampleType);
              }
            } else {
              if (sample->sfSampleType == LEFT_SAMPLE)
                sample_bank
                ...
                sample_bank
                    ->voice_samples_mono[options->opt_bank][wanted_patch]++;
              record_velocity_range(options, sample_bank, 0, options->opt_bank,
                                    wanted_patch, velmin, velmax,
                                    sample->sfSampleType);
            }
          }
        }
...
static char *unsf_concat(const char *s1, const char *s2) {
  ...
  char *result = NULL;
  if (!(result =
            (char *)malloc(len1 + len2 + 1))) { /* +1 for the zero-terminator */
...
static int unsf_mkdir(char *dir) {
  ...
  if (dup_dir[0] == '/' || dup_dir[0] == '\\') absolute_path = 1;
...
static void make_directories(UnSF_Options *options, SampleBank *sample_bank) {
  ...
  if (options->opt_verbose) printf("Making bank directories.\n");

  for (i = 0; i < UNSF_RANGE; i++)
    if (sample_bank->tonebank[i]) tonebank_count++;
  ...
  for (i = 0; i < UNSF_RANGE; i++) {
    if (sample_bank->tonebank[i]) {
      if (tonebank_count > 1) {
        ...
        sample_bank->tonebank_name[i] = strdup(options->basename);
      if (options->opt_no_write) continue;
      directory =
          unsf_concat(options->output_directory, sample_bank->tonebank_name[i]);
  ...
  for (i = 0; i < UNSF_RANGE; i++) {
    if (sample_bank->drumset_name[i]) {
      directory =
          unsf_concat(options->output_directory, sample_bank->drumset_name[i]);
      if (unsf_mkdir(directory) < 0) {
        exit(1); /* FIXME: library must NOT exit() */
      }
      free(directory);
...
static void sort_velocity_layers(UnSF_Options *options,
  ...
  int i, j, k, velmin, velmax, velcount, left_patches, right_patches,
      mono_patches;
  int width, widest;
  VelocityRangeList *vlist;
...
static void mem_write_block(const void *data, int size, unsigned char **mem,
                            int *mem_size, int *mem_alloced) {
...
static void mem_write8(int val, unsigned char **mem, int *mem_size,
                       int *mem_alloced) {
...
static void mem_write16(int val, unsigned char **mem, int *mem_size,
                        int *mem_alloced) {
...
static void mem_write32(int val, unsigned char **mem, int *mem_size,
                        int *mem_alloced) {
  mem_write8(val & 0xFF, mem, mem_size, mem_alloced);
  mem_write8((val >> 8) & 0xFF, mem, mem_size, mem_alloced);
...
static void apply_generator(UnSF_Options *options, SF_Meta *sf_meta,
                            sfGenList *g, int preset, int global) {
  switch (g->sfGenOper) {
    ...
    case SFGEN_keyRange:
      if (preset) {
        if (g->genAmount.ranges.byLo >= sf_meta->keymin &&
            g->genAmount.ranges.byHi <= sf_meta->keymax) {
    ...
    case SFGEN_velRange:
      if (preset) {
        if (g->genAmount.ranges.byLo >= sf_meta->velmin &&
            g->genAmount.ranges.byHi <= sf_meta->velmax) {
    ...
    default:
      fprintf(stderr,
              "Warning: generator %d with value %d not handled at the %s %s "
              "level\n",
              g->sfGenOper, g->genAmount.shAmount, global ? "global" : "local",
...
static void convert_tremolo(SP_Meta *sp_meta, SF_Meta *sf_meta) {
  ...
  sp_meta->tremolo_phase_increment = sp_meta->tremolo_sweep_increment =
      sp_meta->tremolo_depth = 0;
...
static void convert_vibrato(SP_Meta *sp_meta, SF_Meta *sf_meta) {
  ...
  if (sf_meta->delayModLFO)
    sp_meta->delayModLFO = (int)timecent2msec(sf_meta->delayModLFO);
  ...
  if (!shift) {
    sp_meta->vibrato_depth = sp_meta->vibrato_control_ratio =
        sp_meta->vibrato_sweep_increment = 0;
    return;
  }
...
static int calc_root_pitch(SP_Meta *sp_meta, SF_Meta *sf_meta) {
  ...
  */
  ...
  /* it's too high.. */
  if (root >= sf_meta->keymax + 60) root -= 60;
  /*
  ...
  */
  ...
  if (root > 127)
    return (int)((double)freq_table[127] * bend_coarse[root - 127] *
                 bend_fine[tune]);
  else if (root < 0)
    return (int)((double)freq_table[0] / bend_coarse[-root] * bend_fine[tune]);
  else
    return (int)((double)freq_table[root] * bend_fine[tune]);
}

#define CB_TO_VOLUME(centibel) \
  (255 * (1.0 - ((double)(centibel) / 100.0) / (1200.0 * log10(2.0))))
...
#define TO_VOLUME(centibel) \
  (unsigned char)(255 * pow(10.0, -(double)(centibel) / 200.0))
...
static unsigned char calc_sustain(SF_Meta *sf_meta) {
  ...
  if (level < 100)
    level = 250; /* Protect against bogus value? This is for PC42c saxes. */
...
static void calc_resonance(SP_Meta *sp_meta, SF_Meta *sf_meta) {
  ...
  if (sp_meta->resonance < 0) sp_meta->resonance = 0;
}
...
static void calc_cutoff(SP_Meta *sp_meta, SF_Meta *sf_meta) {
  ...
  if (sf_meta->initialFilterFc < 1)
    val = 13500;
  else
    val = sf_meta->initialFilterFc;

  if (val < 0 || val > 24000) val = 19192;

  if (sf_meta->modEnvToFilterFc /*&& sf_meta->initialFilterFc*/) {
    sp_meta->modEnvToFilterFc =
        pow(2.0, ((double)sf_meta->modEnvToFilterFc / 1200.0));
  } else
    sp_meta->modEnvToFilterFc = 0;

  if (sf_meta->modLfoToFilterFc /* && sf_meta->initialFilterFc*/) {
    sp_meta->modLfoToFilterFc =
        pow(2.0, ((double)sf_meta->modLfoToFilterFc / 1200.0));
  } else
    sp_meta->modLfoToFilterFc = 0;

  if (sf_meta->mod_env_to_pitch) {
    sp_meta->modEnvToPitch =
        pow(2.0, ((double)sf_meta->mod_env_to_pitch / 1200.0));
  } else
    sp_meta->modEnvToPitch = 0;

  sp_meta->cutoff_freq = TO_HZ(val);
}

#ifdef LFO_DEBUG
static void convert_lfo(char *name, int program, int banknum, int wanted_bank)
#else

static void convert_lfo(SP_Meta *sp_meta, SF_Meta *sf_meta)
#endif
{
  int freq = 0, shift = 0;
 ...
 */
static int getmodes(UnSF_Options *options, int sf_sustain_mod_env,
                    int sampleFlags, int program, int banknum) {
  int modes;
  ...
  if (options->opt_adjust_sample_flags) {
    if (sampleFlags && sf_sustain_mod_env == 0)
      sampleFlags = 3;
    else if (sampleFlags && sf_sustain_mod_env >= 1000)
      sampleFlags = 1;
    else if (banknum != UNSF_RANGE && sampleFlags == 1) {
      /* organs, accordians */
      if (program >= 16 && program <= 23) sampleFlags = 3;
      /* strings */
      else if (program >= 40 && program <= 44)
        sampleFlags = 3;
      /* strings, voice */
      else if (program >= 48 && program <= 54)
        sampleFlags = 3;
      /* horns, woodwinds */
      else if (program >= 56 && program <= 79)
        sampleFlags = 3;
      /* lead, pad, fx */
      else if (program >= 80 && program <= 103)
        sampleFlags = 3;
      /* bagpipe, fiddle, shanai */
      else if (program >= 109 && program <= 111)
        sampleFlags = 3;
      /* breath noise, ... telephone, helicopter */
      else if (program >= 121 && program <= 125)
        sampleFlags = 3;
      /* applause */
      else if (program == 126)
        sampleFlags = 3;
  ...
  if (sampleFlags == 1 || sampleFlags == 3) modes |= MODES_LOOPING;
  if (sampleFlags == 3) modes |= MODES_SUSTAIN;
  return modes;
}
...
static int grab_soundfont_sample(UnSF_Options *options, char *name, int program,
                                 int banknum, int wanted_bank,
                                 int waiting_list_count,
                                 EMPTY_WHITE_ROOM *waiting_list,
                                 unsigned char **mem, int *mem_alloced,
  ...
  if (options->opt_header) {
    VelocityRangeList *vlist;
    int velcount, velcount_part1, k, velmin, velmax, left_patches,
        right_patches;
    ...
    *mem_size = 0;
    ...
    for (i = 0; i < 60 && sample_bank->cpyrt[i]; i++)
      mem_write8(sample_bank->cpyrt[i], mem, mem_size, mem_alloced);
    ...
    mem_write16(waiting_list_count, mem, mem_size,
                mem_alloced);                     /* number of waveforms */
    ...
    /* List of velocity layers with left and right patch counts. There is room
     * for 10 here. For each layer, give four bytes: velocity min, velocity max,
    ...
    mem_write8(waiting_list_count, mem, mem_size,
               mem_alloced); /* number of samples */
    ...
    if (velcount > velcount_part1) {
      for (k = velcount_part1; k < velcount; k++) {
        if (vlist) {
          ...
          if (vlist->right_patches[k] && !options->opt_mono)
            right_patches =
                vlist->right_patches[k] /* + vlist->mono_patches[k]*/;
          else
            right_patches = 0;
        } else {
          fprintf(stderr, "Internal error.\n");
  ...
  for (n = 0; n < waiting_list_count; n++) {
    ...
    vol = 1.0;
    while (v-- > 0) vol /= pow(10, 0.005);

    waiting_list[n].volume = vol;

    if ((keymin <= 60) && (keymax >= 60)) total_vol += vol;
  }
  ...
  if (total_vol > 0) {
    for (n = 0; n < waiting_list_count; n++)
      waiting_list[n].volume =
          MID(0.2, waiting_list[n].volume / total_vol, 1.0);
  }
  ...
  for (n = 0; n < waiting_list_count; n++) {
    ...
    sf_meta.loop_start =
        MID(0, sf_meta.loop_start - sf_meta.start, sf_meta.end);
    ...
    if (sf_meta.keyscale == 100)
      freq_scale = 1024;
    else
      freq_scale = MID(0, sf_meta.keyscale * 1024 / 100, 2048);

    /* I don't know about this tuning. (gl) */
    /*sf_meta.tune += sf_meta.mod_env_to_pitch * MID(0,
     * 1000-sf_meta.sustain_mod_env, 1000) / 1000;*/
    ...
    */
    ...
    if (waiting_list[n].stereo_mode == LEFT_SAMPLE)
      ...
      mem_write8('0' + waiting_list[n].stereo_mode, mem, mem_size, mem_alloced);
    mem_write8(0, mem, mem_size, mem_alloced);
    ...
    if (options->opt_8bit) {
      ...
      mem_write32(sf_meta.loop_start, mem, mem_size,
                  mem_alloced); /* loop start */
      ...
      mem_write32(sf_meta.loop_start * 2, mem, mem_size,
                  mem_alloced); /* loop start */
      mem_write32(sf_meta.loop_end * 2, mem, mem_size,
                  mem_alloced); /* loop end */
    }

    mem_write16(sample->dwSampleRate, mem, mem_size,
                mem_alloced); /* sample freq */
    ...
    if (options->opt_veryverbose) {
      printf(
          "attack_vol_env=%d, hold_vol_env=%d, decay_vol_env=%d, "
          "release_vol_env=%d, sf_meta.delay=%d\n",
          sf_meta.attack_vol_env, sf_meta.hold_vol_env, sf_meta.decay_vol_env,
          sf_meta.release_vol_env, sf_meta.delay_vol_env);
      printf("iA= %d, sp_volume=%d sustain=%d attack=%d ATTACK=%d\n",
             sf_meta.initialAttenuation, sp_meta.volume, sustain, attack,
             msec2gus(attack, sp_meta.volume));
      printf("\thold=%d r=%d HOLD=%d\n", hold, sp_meta.volume - 1,
             msec2gus(hold, sp_meta.volume - 1));
      ...
      printf("\trelease=%d r=255 RELEASE=%d\n", release,
             msec2gus(release, 255));
      printf("  levels: %d %d %d %d\n", sp_meta.volume, sp_meta.volume - 1,
             sustain, 0);
    }

    mem_write8(msec2gus(attack, sp_meta.volume), mem, mem_size,
               mem_alloced); /* envelope rates */
    mem_write8(msec2gus(hold, sp_meta.volume - 1), mem, mem_size, mem_alloced);
    mem_write8(msec2gus(decay, sp_meta.volume - 1 - sustain), mem, mem_size,
               mem_alloced);
    mem_write8(msec2gus(release, 255), mem, mem_size, mem_alloced);
    mem_write8(0x3F, mem, mem_size, mem_alloced);
    mem_write8(0x3F, mem, mem_size, mem_alloced);

    mem_write8(sp_meta.volume, mem, mem_size,
               mem_alloced); /* envelope offsets */
    ...
    mem_write8(sp_meta.tremolo_sweep_increment, mem, mem_size,
               mem_alloced); /* tremolo sweep */
    mem_write8(sp_meta.tremolo_phase_increment, mem, mem_size,
               mem_alloced); /* tremolo rate */
    mem_write8(sp_meta.tremolo_depth, mem, mem_size,
               mem_alloced); /* tremolo depth */

    convert_vibrato(&sp_meta, &sf_meta);
    mem_write8(sp_meta.vibrato_sweep_increment, mem, mem_size,
               mem_alloced); /* vibrato sweep */
    mem_write8(sp_meta.vibrato_control_ratio, mem, mem_size,
               mem_alloced); /* vibrato rate */
    mem_write8(sp_meta.vibrato_depth, mem, mem_size,
               mem_alloced); /* vibrato depth */
    ...
    flags = getmodes(options, sf_meta.sustain_mod_env, sf_meta.mode, program,
                     wanted_bank);
    ...
    if (options->opt_adjust_volume) {
      ...
      sample_volume = sp_meta.volume;

    mem_write16(sample_volume, mem, mem_size,
                mem_alloced); /* I'm not sure this is here. (gl) */
    ...
    mem_write8(msec2gus(mod_attack, sp_meta.volume), mem, mem_size,
               mem_alloced); /* envelope rates */
    mem_write8(msec2gus(mod_hold, sp_meta.volume - 1), mem, mem_size,
               mem_alloced);
    mem_write8(msec2gus(mod_decay, sp_meta.volume - 1 - mod_sustain), mem,
               mem_size, mem_alloced);
    ...
    mem_write16(sp_meta.lfo_phase_increment, mem, mem_size,
                mem_alloced); /* lfo */
    ...
    if (sf_meta.instrument_unused5 == -1)
      ...
      mem_write8(sf_meta.instrument_unused5, mem, mem_size, mem_alloced);

    if (options->opt_8bit) { /* sample waveform */
      for (i = 0; i < length; i++)
        mem_write8(
            (int)((sf_sample_data[sample->dwStart + i] >> 8) * vol) ^ 0x80, mem,
            mem_size, mem_alloced);
    } else {
      for (i = 0; i < length; i++)
        mem_write16(sf_sample_data[sample->dwStart + i], mem, mem_size,
                    mem_alloced);
    }
  }
  return TRUE;
...
static int grab_soundfont(
    UnSF_Options *options, int num, int drum, char *name, int wanted_velmin,
    int wanted_velmax, int sf_num_presets, sfPresetHeader *sf_presets,
    ...
    unsigned char **mem, int *mem_alloced, int *mem_size, short *sf_sample_data,
    SampleBank *sample_bank) {
  sfPresetBag *pindex;
  sfGenList *pgen;
  sfInst *iheader;
  ...
  for (pnum = 0; pnum < sf_num_presets; pnum++) {
    ...
    if ((sf_presets[pnum].wPreset == wanted_patch) &&
        (sf_presets[pnum].wBank == wanted_bank)) {
      /* find what substructures it uses */
      pindex = &sf_preset_indexes[sf_presets[pnum].wPresetBagNdx];
      pindex_count =
          sf_presets[pnum + 1].wPresetBagNdx - sf_presets[pnum].wPresetBagNdx;

      if (pindex_count < 1) return FALSE;
      ...
      for (inum = 0; inum < pindex_count; inum++) {
        ...
        if (pgen_count > 0 && pgen[pgen_count - 1].sfGenOper !=
                                  SFGEN_instrument) { /* global preset zone */
          global_pzone = pgen;
        ...
        if ((pgen_count > 0) &&
          ...
          for (lnum = 0; lnum < iindex_count; lnum++) {
            igen = &sf_instrument_generators[iindex[lnum].wInstGenNdx];
            igen_count =
                iindex[lnum + 1].wInstGenNdx - iindex[lnum].wInstGenNdx;
            ...
            if (drum && (wanted_keymin < keymin || wanted_keymin > keymax))
              continue;
            if (!drum && (keymin < wanted_keymin || keymax > wanted_keymax))
              continue;
            ...
            if ((igen_count > 0) &&
              ...
              if (sample->sfSampleType == LEFT_SAMPLE &&
                  !options->opt_left_channel)
                continue;
              if (sample->sfSampleType == RIGHT_SAMPLE &&
                  !options->opt_right_channel)
                continue;
              if (sample->sfSampleType == MONO_SAMPLE &&
                  options->opt_right_channel)
                continue;
              ...
              if (waiting_list_count < MAX_WAITING) {
                if (options->opt_veryverbose) printf("  - sample %s\n", s);
                ...
                waiting_list[waiting_list_count].global_izone_count =
                    global_izone_count;
                waiting_list[waiting_list_count].global_pzone_count =
                    global_pzone_count;
                waiting_list[waiting_list_count].volume = 1.0;
                waiting_list[waiting_list_count].stereo_mode =
                    sample->sfSampleType;
                waiting_list_count++;
      ...
      if (waiting_list_count > 0) {
        ...
        VelocityRangeList *vlist;
        if (drum)
          vlist = sample_bank->drum_velocity[wanted_patch][wanted_keymin];
        else
          vlist = sample_bank->voice_velocity[wanted_bank][wanted_patch];
        if (!vlist) {
          fprintf(stderr,
        ...
        for (k = 0; k < vcount; k++)
          if (vlist->velmin[k] == wanted_velmin &&
              vlist->velmax[k] == wanted_velmax)
            break;
        if (k == vcount) {
          fprintf(
              stderr,
        ...
        if (options->opt_right_channel)
          pcount = vlist->right_patches[k];
        else
          pcount = vlist->left_patches[k] + vlist->mono_patches[k];
        if (pcount != waiting_list_count) {
          fprintf(stderr,
                  "\nFor %sinstrument %s %s found %d samples when there should "
                  "be %d samples.\n",
          ...
          fprintf(
              stderr,
              "\tkeymin=%d keymax=%d patch=%d bank=%d, velmin=%d, velmax=%d\n",
              wanted_keymin, wanted_keymax, wanted_patch, wanted_bank,
              wanted_velmin, wanted_velmax);
          fprintf(stderr,
                  "\tleft patches %d, right patches %d, mono patches %d\n",
                  vlist->left_patches[k], vlist->right_patches[k],
        ...
        if (drum)
          return grab_soundfont_sample(
              options, name, wanted_keymin, wanted_patch, wanted_bank,
              waiting_list_count, waiting_list, mem, mem_alloced, mem_size,
              sf_sample_data, sample_bank);
        else
          return grab_soundfont_sample(options, name, wanted_patch, wanted_bank,
                                       wanted_bank, waiting_list_count,
                                       waiting_list, mem, mem_alloced, mem_size,
                                       sf_sample_data, sample_bank);
      } else {
        fprintf(stderr,
                "\nStrange... no valid layers found in instrument %s bank %d "
                "prog %d\n",
                name, drum ? wanted_patch : wanted_bank,
                drum ? wanted_keymin : wanted_patch);
        return FALSE;
      }
    }
  }
...
static void make_patch_files(
    UnSF_Options *options, int sf_num_presets, sfPresetHeader *sf_presets,
    sfPresetBag *sf_preset_indexes, sfGenList *sf_preset_generators,
    sfInst *sf_instruments, sfInstBag *sf_instrument_indexes,
    sfGenList *sf_instrument_generators, sfSample *sf_samples,
    short *sf_sample_data, SampleBank *sample_bank) {
  ...
  if (options->opt_verbose) printf("Melodic patch files.\n");
  for (i = 0; i < UNSF_RANGE; i++) {
    if (sample_bank->tonebank[i]) {
      for (j = 0; j < UNSF_RANGE; j++) {
        if (sample_bank->voice_name[i][j]) {
          ...
          for (k = 0; k < velcount; k++) {
            ...
            if (!grab_soundfont(
                    options, j, FALSE, sample_bank->voice_name[i][j],
                    wanted_velmin, wanted_velmax, sf_num_presets, sf_presets,
                    sf_preset_indexes, sf_preset_generators, sf_instruments,
                    sf_instrument_indexes, sf_instrument_generators, sf_samples,
                    &mem, &mem_alloced, &mem_size, sf_sample_data,
                    sample_bank)) {
              ...
              if (sample_bank->voice_velocity[i][j])
            ...
            if (right_patches && !options->opt_mono) {
              ...
              if (!grab_soundfont(
                      ...
                      wanted_velmin, wanted_velmax, sf_num_presets, sf_presets,
                      sf_preset_indexes, sf_preset_generators, sf_instruments,
                      sf_instrument_indexes, sf_instrument_generators,
          ...
          sprintf(tmpname, "%s/%s.pat", sample_bank->tonebank_name[i],
                  sample_bank->voice_name[i][j]);
      ...
      for (j = 0; j < UNSF_RANGE; j++) {
        if (sample_bank->drum_name[i][j]) {
          ...
          for (k = 0; k < velcount; k++) {
            ...
            options->opt_left_channel = TRUE;
            options->opt_right_channel = FALSE;
            if (!grab_soundfont(options, j, TRUE, sample_bank->drum_name[i][j],
                                wanted_velmin, wanted_velmax, sf_num_presets,
                                sf_presets, sf_preset_indexes,
                                sf_preset_generators, sf_instruments,
                                sf_instrument_indexes, sf_instrument_generators,
                                sf_samples, &mem, &mem_alloced, &mem_size,
                                sf_sample_data, sample_bank)) {
              fprintf(
                  stderr, "Could not create left/mono patch %s for bank %s\n",
                  sample_bank->drum_name[i][j], sample_bank->drumset_name[i]);
              fprintf(stderr, "\tlayer %d of %d layer(s)\n", k + 1, velcount);
              if (sample_bank->drum_velocity[i][j])
            ...
            if (right_patches && !options->opt_mono) {
              ...
              options->opt_right_channel = TRUE;
              if (!grab_soundfont(
                      options, j, TRUE, sample_bank->drum_name[i][j],
                      wanted_velmin, wanted_velmax, sf_num_presets, sf_presets,
                      sf_preset_indexes, sf_preset_generators, sf_instruments,
                      sf_instrument_indexes, sf_instrument_generators,
                      sf_samples, &mem, &mem_alloced, &mem_size, sf_sample_data,
          ...
          if (abort_this_one || options->opt_no_write) continue;
          sprintf(tmpname, "%s/%s.pat", sample_bank->drumset_name[i],
                  sample_bank->drum_name[i][j]);
          file_path = unsf_concat(options->output_directory, tmpname);
          if (!(pf = fopen(file_path, "wb"))) {
            fprintf(stderr, "\nCould not open patch file %s\n", file_path);
...
static void gen_config_file(UnSF_Options *options, SampleBank *sample_bank) {
  ...
  if (options->opt_verbose) printf("Generating config file.\n");
    ...
    if (sample_bank->tonebank[i]) {
      ...
      for (j = 0; j < UNSF_RANGE; j++) {
        if (sample_bank->voice_name[i][j]) {
          ...
          if (velcount > 1)
            fprintf(options->cfg_fd, "\t# %d velocity ranges", velcount);
          if (right_patches) {
            if (velcount == 1)
              fprintf(options->cfg_fd, "\t# stereo");
            else
              fprintf(options->cfg_fd, ", stereo");
          }
          fprintf(options->cfg_fd, "\n");
  ...
  for (i = 0; i < UNSF_RANGE; i++) {
    if (sample_bank->drumset_name[i]) {
      fprintf(options->cfg_fd, "\ndrumset %d #N %s\n", i,
              sample_bank->drumset_short_name[i]);
      for (j = 0; j < UNSF_RANGE; j++) {
        if (sample_bank->drum_name[i][j]) {
          vlist = sample_bank->drum_velocity[i][j];
          if (vlist) {
            ...
            case CID_INFO:
              ...
              switch (subchunk.id) {
                case CID_ifil:
                  if (get16(f) < 2) {
                    fprintf(stderr,
                            "Error: this is a SoundFont 1.x file, and I only "
                            "understand version 2 (.sf2)\n");
                    rc = -1;
                    goto getout;
                  }
                  get16(f);
                ...
                case CID_INAM:
                  print_sf_string(options, f,
                                  "Bank name:", options->opt_no_write,
                                  &sample_bank);
                  break;

                case CID_irom:
                  print_sf_string(options, f,
                                  "ROM name:", options->opt_no_write,
                                  &sample_bank);
                  break;
                ...
                case CID_ICOP:
                  print_sf_string(options, f,
                                  "Copyright:", options->opt_no_write,
                                  &sample_bank);
                  break;
            ...
            case CID_pdta:
              ...
              switch (subchunk.id) {
                case CID_phdr:
                  ...
                  sf_presets = (sfPresetHeader *)malloc(sizeof(sfPresetHeader) *
                                                        sf_num_presets);
                ...
                case CID_pbag:
                  ...
                  sf_preset_indexes = (sfPresetBag *)malloc(
                      sizeof(sfPresetBag) * sf_num_preset_indexes);
                ...
                case CID_pgen:
                  ...
                  sf_preset_generators = (sfGenList *)malloc(
                      sizeof(sfGenList) * sf_num_preset_generators);
                ...
                case CID_inst:
                  ...
                  sf_instruments =
                      (sfInst *)malloc(sizeof(sfInst) * sf_num_instruments);
                ...
                case CID_ibag:
                  ...
                  sf_instrument_indexes = (sfInstBag *)malloc(
                      sizeof(sfInstBag) * sf_num_instrument_indexes);
                ...
                case CID_igen:
                  ...
                  sf_instrument_generators = (sfGenList *)malloc(
                      sizeof(sfGenList) * sf_num_instrument_generators);
                ...
                case CID_shdr:
                  ...
                  sf_samples =
                      (sfSample *)malloc(sizeof(sfSample) * sf_num_samples);
            ...
            case CID_sdta:
              ...
              switch (subchunk.id) {
                case CID_smpl:
                  ...
                  sf_sample_data =
                      (short *)malloc(sizeof(short) * sf_sample_data_size);
...
getout:
  ...
  if (rc == 0) {
    if ((!sf_sample_data) || (!sf_presets) || (!sf_preset_indexes) ||
        (!sf_preset_generators) || (!sf_instruments) ||
        (!sf_instrument_indexes) || (!sf_instrument_generators) ||
        (!sf_samples))
      BAD_SF();
    ...
    grab_soundfont_banks(options, sf_num_presets, sf_presets, sf_preset_indexes,
                         sf_preset_generators, sf_instruments,
                         sf_instrument_indexes, sf_instrument_generators,
                         sf_samples, &sample_bank);
    make_directories(options, &sample_bank);
    sort_velocity_layers(options, &sample_bank);
    shorten_drum_names(&sample_bank);
    make_patch_files(options, sf_num_presets, sf_presets, sf_preset_indexes,
                     sf_preset_generators, sf_instruments,
                     sf_instrument_indexes, sf_instrument_generators,
                     sf_samples, sf_sample_data, &sample_bank);
...
UNSF_SYMBOL UnSF_Options unsf_initialization(void) {
  UnSF_Options options = {0, 0, 0, 0, 0, 0, 0,    0,    NULL,
